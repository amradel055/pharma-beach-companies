<template>
  <div class="terms-page">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">شروط وأحكام الموقع</h1>
        <p class="page-desc">تعديل الشروط والأحكام العامة للموقع</p>
      </div>
...
const arContent = ref(settings.siteTerms.ar?.content || '')
const enContent = ref(settings.siteTerms.en?.content || '')

const currentStatus = computed(() => settings.siteTerms.ar?.status || 'draft')
...
function saveAs(status) {
  ...
  settings.save()
  toast.success(status === 'published' ? 'تم نشر الشروط والأحكام' : 'تم حفظ المسودة')
}
</script>
